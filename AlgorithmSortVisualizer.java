...
public abstract class AlgorithmSortVisualizer extends JFrame {
	...
	protected JTextField createStyledTextField(int columns) {
		...
		field.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(ACCENT_COLOR, 1, true),
	...
	protected JButton createStyledButton(String text) {
		...
		button.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(ACCENT_COLOR, 1, true),
	...
	protected Border createRoundedBorder(String title) {
		return BorderFactory.createCompoundBorder(
				BorderFactory.createTitledBorder(BorderFactory.createLineBorder(new Color(200, 200, 200), 1, true),
						title, TitledBorder.LEFT, TitledBorder.TOP, HEADER_FONT, ACCENT_COLOR),
	...
	protected Point calculateBezierPoint(Point start, Point end, double t, int arcHeight) {
		Point control = new Point((start.x + end.x) / 2, Math.min(start.y, end.y) - arcHeight);

		int x = (int) (Math.pow(1 - t, 2) * start.x + 2 * (1 - t) * t * control.x + Math.pow(t, 2) * end.x);
		int y = (int) (Math.pow(1 - t, 2) * start.y + 2 * (1 - t) * t * control.y + Math.pow(t, 2) * end.y);
